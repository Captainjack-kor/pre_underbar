...
_.uniq = function (arr) {
  // TODO: 여기에 코드를 작성합니다.
  //arr의 각요소를 넣어주고 요소가 새로운 배열에 존재한다면 안넣고 
  let result=[];
  
  let uniqFunc = function(el){
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  if(initVal!==undefined){
    ...
    _.each (result, function(item,index){
      acc=iteratee(acc,item,index,result);
    });
  }
  return acc;
};
