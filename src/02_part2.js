...
// _.every는 배열의 모든 요소가 test 함수(iteratee)를 통과하면 true를, 
...
_.every = function (arr, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
  // arr의 각요소에 iteratee함수 실행한 결과가 모두 true인경우 true리턴
  if(arr.length===0){
    return true;
  }
...
// _.some은 배열의 요소 중 하나라도 test 함수(iteratee)를 통과하면 true를, 
// 그렇지 않은 경우 false를 리턴합니다.
...
_.extend = function (obj1, ...objs) {
  // TODO: 여기에 코드를 작성합니다.
  //객체더하기 ojb1[key] = value
  
  //obj1에 [obj(n)의 키][i] = obj(n)의 밸류
...
_.zip = function (...arrs) {
  // TODO: 여기에 코드를 작성합니다.
  // arrs = [arr1, arr2, arr(n)] 요소의 길이를 모른다.
  let maxLength = _.reduce(arrs, function(acc,cur){
  ...
  return result;
  /*
  _.each(arrs,function(el, iterateeNum){
    let arr = [];
    별도의 배열로 들어간다.
    arr.push(el[iterateeNum]);
    result.push(arr);
  })
  return result;
...
// 최종적으로 리턴되는 배열의 각 요소의 길이는 입력으로 
// 전달되는 배열 중 가장 '짧은' 배열의 길이로 통일됩니다.
...
_.difference = function (...arrs) {
  ...
  let result = [];
  let count = 1;
  // 반복 : 첫 번째 배열의 0~i번째 엘리먼트
  for (let i = 0; i < arrs[0].length; i++) {
    // 반복 : 나머지 각 배열 (1번부터 arrs.length-1)
    for ( let j = 1; j < arrs.length; j++) {
      // 첫번째 배열의 엘리먼트가 해당 각 배열에 포함 되어있으면
      if (_.includes(arrs[j], arrs[0][i]))
        count ++;
    }
    if ( count < 2) {
...
_.sortBy = function (arr, transform, order) {
  ...
  if (typeof arr[0] === 'number') {
    result = arr.sort(function compareNumbers(a, b) {
      return a - b;
    })
  ...
  // transform이 있는 경우
  let mapArr = _.map(arr, transform);
  result = mapArr.sort(function compareNumbers(a, b) {
    return a - b;
  });
  // order가 내림차순인 경우
  if (order === -1) {
